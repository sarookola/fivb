...
//        System.out.println("child: "+child + "      getDrawS: "+getDrawSettings()[child].getDrawingDimsByIndex(currentChildDimIndex).length+"  currenf: "+currentFrame);
        ...
        double x2 = x1;
        if (getDrawSettings()[child].getDrawingDimsByIndex(currentChildDimIndex).length > currentFrame + 1) {
            x2 = getDrawSettings()[child].getDrawingDimsByIndex(currentChildDimIndex)[currentFrame + 1].getWidthRelative();
        }
    ...
    public int reanimateobjectsByparentGetHscale(DrawItemDisplay parent, int child, int currentFrame, double curentframeDivAllframs) {
        ...
        double x2 = x1;
        if (getDrawSettings()[child].getDrawingDimsByIndex(currentChildDimIndex).length > currentFrame + 1) {
            x2 = getDrawSettings()[child].getDrawingDimsByIndex(currentChildDimIndex)[currentFrame + 1].getHeightRelative();
        }
    ...
    public int reanimateobjectsByparentGetXpos(DrawItemDisplay parent, int child, double childWScale, int currentFrame, double curentframeDivAllframs) {
        ...
        double x2 = x1;
        if (getDrawSettings()[child].getDrawingDimsByIndex(currentChildDimIndex).length > currentFrame + 1) 
            x2 = getDrawSettings()[child].getDrawingDimsByIndex(currentChildDimIndex)[currentFrame + 1].getx();
...
//        return(xn);
    ...
    public int reanimateobjectsByparentGetYpos(DrawItemDisplay parent, int child, double childHScale, int currentFrame, double curentframeDivAllframs) {
        ...
        double x2 = x1;
        if (getDrawSettings()[child].getDrawingDimsByIndex(currentChildDimIndex).length > currentFrame + 1) {
            x2 = getDrawSettings()[child].getDrawingDimsByIndex(currentChildDimIndex)[currentFrame + 1].gety();
        }
...
//        System.out.print("\tp h="+parent.getHeightRelative()+"\tc h="+childHScale+"\tyn=" + yn+" \tparent y ="+(parent.getyPos())+" \tynn="+ynn);
    ...
    public int reanimateobjectsByparentGetOpac(DrawItemDisplay parent, int child, int currentFrame, double curentframeDivAllframs) {
        ...
        double x2 = x1;
        if (getDrawSettings()[child].getDrawingDimsByIndex(currentChildDimIndex).length > currentFrame + 1) {
            x2 = getDrawSettings()[child].getDrawingDimsByIndex(currentChildDimIndex)[currentFrame + 1].getopac();
        }
        double on = (x1 + ((x2 - x1) * curentframeDivAllframs));
        return (int) (on * parent.getOpac() / 255);
    }
    
    public void readold(SettingPackOld   settingold){
        for (int i = 0; i < drawSettingS.length; i++) {
            DrawSettingS drawSettingS1 = drawSettingS[i];
            List<DrawSettingDims[]> drawingDimsList =  new LinkedList<DrawSettingDims[]>();
            drawingDimsList.add (settingold.getDrawSettingsOld()[i].getDrawinGObject());
            drawSettingS1.setDrawingDimsList(drawingDimsList );
        }
    }
